...
public class Board{
    ...
    static final float NON_REALTIME_DRAWRATE = 10; // draw once every NON_REALTIME_DRAWRATE years if playspeed is >= MAX_REALTIME_PLAYSPEED
  ...
  float[] letterFrequencies = {8.167f,1.492f,2.782f,4.253f,12.702f,2.228f,2.015f,6.094f,6.966f ,0.153f,0.772f,4.025f,2.406f,6.749f,7.507f,1.929f,0.095f,5.987f,6.327f,9.056f,2.758f,0.978f,2.361f,0.150f,1.974f,10000.0f};//0.074};
  ...
  public Board(int w, int h, float stepSize, float min, float max, int rta, int cm, int SEED, String INITIAL_FILE_NAME, double ts){
    ...
    for(int i = 0; i < ROCKS_TO_ADD; i++){
      rocks.add(new SoftBody(EvolvioMod.main.random(0,boardWidth),EvolvioMod.main.random(0,boardHeight),0,0,getRandomSize(),ROCK_DENSITY,EvolvioMod.main.hue(ROCK_COLOR),EvolvioMod.main.saturation(ROCK_COLOR),EvolvioMod.main.brightness(ROCK_COLOR),this,year));
    }
  ...
  public void drawUI(float scaleUp,double timeStep, int x1, int y1, int x2, int y2, PFont font){
    ...
    if(selectedCreature == null){
      ...
      for(int i = 0; i < LIST_SLOTS; i++){
        if(list[i] != null){
          ...
          if(list[i].energy > 1){
        	  ...
        	  EvolvioMod.main.rect(85+(float)(multi/maxEnergy),y+5,(float)(multi*(list[i].energy-1)/maxEnergy),25);
          }
          EvolvioMod.main.fill(0,0,1);
          EvolvioMod.main.text(list[i].getCreatureName()+" ["+list[i].id+"] ("+toAge(list[i].birthTime)+")",90,y);
          EvolvioMod.main.text("Energy: "+EvolvioMod.main.nf(100*(float)(list[i].energy),0,2),90,y+25);
        }
      }
      EvolvioMod.main.noStroke();
      ...
      "Screenshot now","-   Image every "+EvolvioMod.main.nf((float)imageSaveInterval,0,2)+" years   +",
      "Text file now","-    Text every "+EvolvioMod.main.nf((float)textSaveInterval,0,2)+" years    +",
      ...
      for(int i = 0; i < 8; i++){
        ...
        if(i == 0){
        	...
        	EvolvioMod.main.text("-"+creatureMinimumIncrement+
      ...
      EvolvioMod.main.text("Energy: "+EvolvioMod.main.nf(100*(float)selectedCreature.energy,0,2)+" yums",10,250);
    ...
    drawThermometer(-45,30,20,660,temperature,THERMOMETER_MIN,THERMOMETER_MAX,EvolvioMod.main.color(0,1,1));
    EvolvioMod.main.popMatrix();
        ...
        if(me == selectedCreature){
          if(EvolvioMod.main.keyPressed){
             if (EvolvioMod.main.key == EvolvioMod.main.CODED) {
              if (EvolvioMod.main.keyCode == EvolvioMod.main.UP) me.accelerate(0.04,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
              if (EvolvioMod.main.keyCode == EvolvioMod.main.DOWN) me.accelerate(-0.04,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
              if (EvolvioMod.main.keyCode == EvolvioMod.main.LEFT) me.turn(-0.1,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
              if (EvolvioMod.main.keyCode == EvolvioMod.main.RIGHT) me.turn(0.1,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
            }else{
              if(EvolvioMod.main.key == ' ') me.eat(0.1,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
              if(EvolvioMod.main.key == 'v') me.eat(-0.1,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
              if(EvolvioMod.main.key == 'f')  me.fight(0.5,timeStep*OBJECT_TIMESTEPS_PER_YEAR);
  ...
  void maintainCreatureMinimum(boolean choosePreexisting){
    while(creatures.size() < creatureMinimum){
      if(choosePreexisting){
        ...
        creatures.add(new Creature(EvolvioMod.main.random(0,boardWidth),EvolvioMod.main.random(0,boardHeight),0,0,
        		EvolvioMod.main.random(MIN_CREATURE_ENERGY,MAX_CREATURE_ENERGY),1,EvolvioMod.main.random(0,1),1,1,
        this,year,EvolvioMod.main.random(0,2*EvolvioMod.main.PI),0,"","[PRIMORDIAL]",true,null,null,1,EvolvioMod.main.random(0,1)));
      }
    }
  }
  ...
  public String[] toBigString(){ // Convert current evolvio board into string. Does not work
    String[] placeholder = {"This feature","Not yet implemented", "see line ~544 in Board's toBigString() function"};
    return placeholder;
  }

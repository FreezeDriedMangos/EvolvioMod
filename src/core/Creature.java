...
//  final double FOOD_SENSITIVITY = 0.3;
  ...
  public void drawBrain(PFont font, float scaleUp, int mX, int mY){
    ...
    EvolvioMod.main.rect((-1.7f-neuronSize)*scaleUp,-neuronSize*scaleUp,(2.4f+BRAIN_WIDTH+neuronSize*2)*scaleUp,(BRAIN_HEIGHT+neuronSize*2)*scaleUp);
    ...
    for(int y = 0; y < BRAIN_HEIGHT; y++){
    	...
    	EvolvioMod.main.text(outputLabels[y],(BRAIN_WIDTH-1+neuronSize+0.1f)*scaleUp,(y+(neuronSize*0.6f))*scaleUp);
    }
    ...
    for(int x = 0; x < BRAIN_WIDTH; x++){
      for(int y = 0; y < BRAIN_HEIGHT; y++){
        ...
        EvolvioMod.main.text(EvolvioMod.main.nf((float)val,0,1),x*scaleUp,(y+(neuronSize*0.6f))*scaleUp);
      }
    }
  ...
  public void drawSoftBody(float scaleUp, float camZoom, boolean showVision){
    ...
    if(showVision){
      for(int i = 0; i < visionAngles.length; i++){
        ...
        EvolvioMod.main.stroke((float)(visionResults[i*3]),(float)(visionResults[i*3+1]),(float)(visionResults[i*3+2]));
        EvolvioMod.main.strokeWeight(board.CREATURE_STROKE_WEIGHT);
        EvolvioMod.main.line((float)((visionOccludedX[i]-CROSS_SIZE)*scaleUp),(float)((visionOccludedY[i]-CROSS_SIZE)*scaleUp),
    ...
    if(fightLevel > 0){
    	...
    	EvolvioMod.main.ellipse((float)(px*scaleUp),(float)(py*scaleUp),(float)(FIGHT_RANGE*radius*scaleUp),(float)(FIGHT_RANGE*radius*scaleUp));
    }
    EvolvioMod.main.strokeWeight(board.CREATURE_STROKE_WEIGHT);
    ...
    if(showVision){
    	...
    	EvolvioMod.main.textAlign(EvolvioMod.main.CENTER);
    	EvolvioMod.main.text(getCreatureName(),(float)(px*scaleUp),(float)((py-getRadius()*1.4-0.07)*scaleUp));
    }
  }
  ...
  public void eat(double attemptedAmount, double timeStep){
      ...
      double amount = ModLoader.apiCreatureEatBehavior.getCreatureEatAmount(this, coveredTile, attemptedAmount, timeStep);
    ...
    if(babySize >= 0){
      ...
      for(int i = 0; i < colliders.size(); i++){
        ...
        if(possibleParent.isCreature && ((Creature)possibleParent).neurons[BRAIN_WIDTH-1][9] > -1){ // Must be a WILLING creature to also give birth.
          float distance = EvolvioMod.main.dist((float)px,(float)py,(float)possibleParent.px,(float)possibleParent.py);
      ...
      if(availableEnergy > babySize){
        ...
        double newPY = EvolvioMod.main.random(-0.01f,0.01f); //To avoid landing directly on parents, resulting in division by 0)
        double newHue = 0;
        ...
        for(int x = 0; x < BRAIN_WIDTH-1; x++){
          for(int y = 0; y < BRAIN_HEIGHT; y++){
            for(int z = 0; z < BRAIN_HEIGHT-1; z++){
              float axonAngle = (float) (Math.atan2((y+z)/2.0-BRAIN_HEIGHT/2.0,x-BRAIN_WIDTH/2)/(2*Math.PI)+Math.PI);
        ...
        for(int x = 0; x < BRAIN_WIDTH; x++){
          for(int y = 0; y < BRAIN_HEIGHT; y++){
            float axonAngle = (float) (Math.atan2(y-BRAIN_HEIGHT/2.0,x-BRAIN_WIDTH/2)/(2*Math.PI)+Math.PI);
            Creature parentForAxon = parents.get((int)(((axonAngle+randomParentRotation)%1.0)*parentsTotal));
        ...
        board.creatures.add(new Creature(newPX,newPY,0,0,
          babySize,density,newHue,newSaturation,newBrightness,board,board.year,EvolvioMod.main.random(0,(float)(2*Math.PI)),0,
